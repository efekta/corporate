...
function youTuber() 
{
	...
	qw = [3840, 2560, 1900, 1200, 820, 620, 380]; // video sizes
	qh = [2160, 1400, 1060, 700, 480, 360, 240];
	...
	function sizeContainer() 
	{
		if ($(video).is(":visible")) 
		{
            ...
            var left = (ww-fw)/2;
            console.log('ww-fw '+left);
			$(video).css("left", left);

            var top = (wh-fh)/2 ;
            console.log('wh-fh '+top);
			$(video).css("top", top);
	...
	$(".youTuber").click(function() // link click
	{
		...
		$(bg).fadeIn("fast");
		// $(bg).append($("<div />").attr({ id: 'youTuberLoading'}).text("Р—Р°РіСЂСѓР·РєР°"));
